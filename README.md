...
- 주요 API 구현 완료
  - 강의: 생성, 목록 조회, 상세 조회, 상태 변경
  - 수강 신청: 생성, 확정, 취소, 내 신청 목록 조회
- 요청 사용자 정보는 `X-User-Id`, `X-User-Role` 헤더로 처리
- Flyway 마이그레이션, DB 제약, 입력 검증, 전역 예외 처리 적용
- 동시성 테스트 포함
  - 마지막 좌석 경쟁
  - 같은 학생의 중복 요청
  - 확정과 취소 동시 요청
- `./mvnw.cmd test` 기준 103개 테스트 통과, 실패 0건
...
- `course`: 강의 도메인 API, 서비스, 엔티티, 저장소, DTO
- `enrollment`: 수강 신청 도메인 API, 서비스, 엔티티, 저장소, DTO
- `common/config`: 헤더 기반 사용자 해석
- `common/error`: 에러 코드, 에러 응답, 전역 예외 처리
...
- 2026-04-27 기준 결과: 테스트 103건, 실패 0건, 에러 0건
...
- 현재 구현은 리소스 소유권 검사보다 역할 기반 인가에 가깝습니다.
...
- 마지막 좌석 경쟁: 정원이 1명일 때 동시에 여러 명이 신청하면 1명만 성공
- 같은 학생의 중복 요청: 동시에 여러 번 요청해도 활성 신청은 1건만 생성
- 확정과 취소 동시 요청: 최종 상태는 유효한 한 가지 상태로만 남음
...
- 권한 처리는 명시적이고 가볍지만, 조직/소유권 기반의 세밀한 권한 모델은 아닙니다.
- 결제는 별도 도메인 없이 상태 변경으로만 표현했습니다.

## AI 활용 범위

본 과제에서는 ChatGPT(GPT-4o mini, 웹 브라우저)를 아래 두 가지 목적으로만 활용했습니다.
...
- 동시성 테스트는 성공/실패 HTTP 상태와 최종 DB 상태는 검증하지만, 실패 케이스의 정확한 에러 코드까지는 아직 검증하지 않습니다.
- 락 타임아웃은 설정되어 있지만, 락 타임아웃 전용 API 에러 매핑은 아직 없습니다.
- 현재 권한 모델은 역할 중심입니다. 과제 요구사항이 강의 생성자 본인만 관리 가능해야 한다면 추가 소유권 검사가 필요합니다.
