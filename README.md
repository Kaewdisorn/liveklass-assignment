...
처음 clone한 뒤 실행 권한이 없다면 아래처럼 한 번만 부여하면 됩니다.

```bash
chmod +x mvnw run-dev.sh run-dev-clean.sh seed.sh
```

```bash
# 1. PostgreSQL + 애플리케이션 실행 (Linux/macOS/Git Bash)
./run-dev.sh
...
처음부터 깨끗한 DB로 시작하고 싶다면 `./run-dev-clean.sh`를 사용할 수 있습니다.
...
```
run-dev.sh                          # PostgreSQL 기동 후 앱 실행
run-dev-clean.sh                    # PostgreSQL 볼륨까지 초기화 후 앱 실행
seed.sh                             # docker/seed.sql 더미 데이터 적재
...
docker/
├── postgres-compose.yml             # 로컬 PostgreSQL 실행 정의
└── seed.sql                         # 샘플 course / enrollment 데이터
```
...
- Bash 실행 환경 (`run-dev.sh`, `run-dev-clean.sh`, `seed.sh` 사용 시: Linux/macOS/Git Bash/WSL)
...
- `./run-dev.sh` : PostgreSQL 컨테이너를 올리고 health check가 통과할 때까지 기다린 뒤 애플리케이션을 실행합니다.
- `./run-dev-clean.sh` : 기존 PostgreSQL 컨테이너와 볼륨을 삭제해 DB를 초기화한 뒤 다시 띄우고 애플리케이션을 실행합니다.
- `./seed.sh` : 실행 중인 `liveklass-postgres` 컨테이너에 `docker/seed.sql`을 적용해 더미 강의/수강 신청 데이터를 적재합니다.
